...
def processJudge(baseDate):
  #只有趋势为null的时候再执行
  cursor.execute("SELECT stock_code,stock_name,yesterday_price FROM stock_day_base_Info_imitate WHERE    DATE(create_time)='%s' AND trend is NULL " % (baseDate))
...
def processSinbleJudge(code,name,yesterDayPrice,baseDate):
  ...
  endTime = baseDate + "09:40"
  openTime = baseDate + "09:31"
   
  print("======"+code)
  cursor.execute("SELECT start_price FROM stock_data_imitate WHERE stock_code ='%s'  AND now_time = '%s'" % (code,openTime))
  startPriceFetch = cursor.fetchone()
  if startPriceFetch:
     startPrice = startPriceFetch[0]
  else:
    return
  cursor.execute("SELECT min(lowest_price) FROM stock_data_imitate WHERE stock_code ='%s'  AND now_time BETWEEN '%s' AND '%s'" % (code,startTime,endTime))
  minPrice = cursor.fetchone()
  cursor.execute("SELECT max(highest_price) FROM stock_data_imitate WHERE stock_code = '%s'  AND now_time BETWEEN '%s' AND '%s' " % (code,startTime,endTime))
  maxPrice = cursor.fetchone()
  amplitude = round(((maxPrice[0]-minPrice[0])/yesterDayPrice)*100,2)
  # 小于2.5 不考虑 .5-3 系数为2   3-4 系数为1.8  
  cursor.execute("SELECT count(*) FROM stock_data_imitate WHERE stock_code = '%s'  AND now_time BETWEEN '%s' AND '%s' AND start_price<=end_price" % (code,startTime,endTime))
  ...
  cursor.execute("SELECT (SELECT now_time FROM stock_data_imitate WHERE stock_code = '%s'  AND now_time BETWEEN '%s' AND '%s' AND highest_price='%s' order by now_time desc limit 1) >= (SELECT now_time FROM stock_data_imitate WHERE stock_code = '%s'  AND now_time BETWEEN '%s' AND '%s' AND lowest_price='%s' order by now_time desc limit 1) " % (code,startTime,endTime,maxPrice[0],code,startTime,endTime,minPrice[0]))
  ...
  effectRow = cursor.execute("UPDATE stock_day_base_Info_imitate set start_price='%s', pre_max_amplitude='%s',pre_low_price='%s',pre_high_price='%s',pre_sell_point='%s',pre_buy_point='%s', trend='%s',amplitude='%s', update_time=now() WHERE stock_code='%s' " % (startPrice,preMaxAmplitude,preLowPrice,preHighPrice,preSellPoint,preBuyPoint,trendFlag,amplitude,code))
  db.commit() 
