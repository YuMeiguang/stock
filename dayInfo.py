...
#baseDate='2021-06-18'
#endTime  = '2021-06-18 15:00:00'

def execute(baseDate):
  #插入当前没有的股票
  cursor.execute("insert into hold_stock_imitate(stock_code,stock_name,interface_code,create_time,update_time) select stock_code,stock_name,simple_head,now(),now() from all_stock where stock_code not in(select stock_code from hold_stock_imitate) and stock_code not like '688%' ")
  db.commit()
