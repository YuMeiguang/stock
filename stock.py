...
def processSingleExecute(num):
  ...
  for holdStock in holdStockList:
    ...
    prosessSinbleStockAllInfo(stockCode,code,holdStock[1],yesterDayPrice,holdStock[3])
...
def prosessSinbleStockAllInfo(stockCode,code,stockName,yesterDayPrice,holdFlag):
  ...
  for data in dataArray:
     ...
     avgPrice = (float(startPrice)+float(endPrice)+float(currentHighestPrice)+float(currentLowestPrice))/4
     cursor = db.cursor()
     ...
     if currentAmplitude >= 1:
       ...
       cursor.execute("INSERT INTO minute_level (stock_code, stock_name, rise_point, down_point, current_amplitude_rise, current_latitude, now_time,deal_date,hold_flag) VALUES (%s, %s, %s, %s, %s, %s, %s,%s,%s)",(stockCode,stockName,str(risePoint),str(downPoint),str(round(currentAmplitude,2)),str(round(currentLatitude,2)),nowTime,baseDate,str(holdFlag)))
       db.commit()
     if currentAmplitude <= -1:
       downPoint = downPoint+1
       cursor.execute("INSERT INTO minute_level (stock_code, stock_name, rise_point, down_point, current_amplitude_down,current_latitude, now_time,deal_date,hold_flag) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",(stockCode,stockName,str(risePoint),str(downPoint),str(round(currentAmplitude,2)),str(round(currentLatitude,2)),nowTime,baseDate,str(holdFlag)))
       db.commit()
       if holdFlag>0:
         smsVerifySend(stockCode,nowTime,baseDate)
