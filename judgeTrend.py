...
def processLianBanStock():
  cursor.execute("SELECT stock_code,stock_name FROM hold_stock_day_info where rize_amplitude>9 and stock_code not like '688%%'  and deal_date='2021-04-09' ")
  ...
  for raisingLimit in raisingLimitStockArray:
    ...
    cursor.execute("SELECT * FROM stock_picking where create_date='%s' and classify='%s' and stock_code='%s'" % ('2021-04-09',3,stockCode))
    pickingStockExistFlag =  cursor.fetchone() 
    if pickingStockExistFlag is None:
      cursor.execute("insert into stock_picking(stock_code, stock_name, create_date, classify, description) VALUES ('%s','%s','%s',3,'%s')" % (stockCode,raisingLimit[1],'2021-04-09',i))
      db.commit() 
  print(dictData)
