...
def processCorrect():
  cursor.execute("select deal_date from trade_days where DATE(now())>=deal_date ORDER BY deal_date desc limit 1")
  dealDataArray = cursor.fetchall()
  global baseDate
  baseDate = str(dealDataArray[0][0]) + ' '
  #baseDate = '2020-11-17 '

  cursor.execute("SELECT stock_code,stock_name,yesterday_price FROM stock_day_base_info  where modify_flag IS NULL")
...
def correctProcess(code,stockName,yesterDayPrice,baseDate):
  ...
  #十点前最高
  cursor.execute("SELECT max(highest_price) FROM stock_data WHERE stock_code='%s' AND now_time between '%s' and '%s' " % (code,startTime1,endTime1))
  highestPrice1 = cursor.fetchone()[0]
  #十点前最低
  cursor.execute("SELECT min(lowest_price) FROM stock_data WHERE stock_code='%s' AND now_time between '%s' and '%s' " % (code,startTime1,endTime1))
  lowestPrice1 = cursor.fetchone()[0]
  #十点到十点半前最高
  cursor.execute("SELECT max(highest_price) FROM stock_data WHERE stock_code='%s' AND now_time between '%s' and '%s' " % (code,startTime2,endTime2))
  highestPrice2 = cursor.fetchone()[0]
  #十点到十点半前最低
  cursor.execute("SELECT min(lowest_price) FROM stock_data WHERE stock_code='%s' AND now_time between '%s' and '%s' " % (code,startTime2,endTime2))
  lowestPrice2 = cursor.fetchone()[0]
  ############  计算结束
  

  #############趋势修正 开始######################  
  cursor.execute("UPDATE stock_day_base_info set modify_flag=0  WHERE stock_code='%s' AND create_time='%s'  " % (code,baseDate))
  db.commit()
  
  cursor.execute("SELECT yesterday_price,trend FROM stock_day_base_info WHERE stock_code='%s' AND DATE(create_time)='%s'" % (code,baseDate))
  baseInfo = cursor.fetchone()
  yesterDayPrice = baseInfo[0]
  trend = baseInfo[1]
  pointNum = 0
  #趋势上涨逆转为向下趋势
  cursor.execute("SELECT count(*) FROM stock_data WHERE stock_code='%s' AND (end_price<=start_price)=1 AND now_time BETWEEN '%s' AND '%s'" % (code,startTime2,endTime2))
